...
def planner_process_image():
    """
    Planner image pipeline for one image URL:
    1) Download image URL
    2) Upload to classmate image API
    3) Apply transform action
    """
    body = request.get_json(silent=True) or {}
    ...
    files = {"file": ("planner-source.jpg", image_resp.content, content_type)}
    try:
